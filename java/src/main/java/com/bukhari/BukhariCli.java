 ...
 *
 * <p>
 * Build an executable fat-JAR with Maven:
 * 
 * <pre>
...
public class BukhariCli {
    ...
    private static String c(String col, String t) {
        return col + t + RESET;
    }
    ...
    private static String cyan(String t) {
        return c(CYAN, t);
    }

    private static String yellow(String t) {
        return c(YELLOW, t);
    }

    private static String magenta(String t) {
        return c(MAGENTA, t);
    }

    private static String gray(String t) {
        return c(GRAY, t);
    }

    private static String red(String t) {
        return c(RED, t);
    }
    ...
    public static void main(String[] args) {
        // Force UTF-8 output so the Prophet's symbol (\u2648) and Arabic render
        // correctly
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "--chapter":
                    if (i + 1 < args.length) {
                        try {
                            chapterId = Integer.parseInt(args[++i]);
                        } catch (NumberFormatException e) {
                            err("Invalid chapter id: " + args[i]);
                        }
                    }
                    break;
                default:
        ...
        if (chapterId != null) {
            ...
            System.out.println(DIV2);
            for (Hadith h : ch)
                printHadith(h, chaptersMap, showArabic, showEnglish, null);
            System.exit(0);
        }
        ...
        if (searchQuery != null) {
            ...
            for (Hadith h : toShow)
                printHadith(h, chaptersMap, showArabic, showEnglish, searchQuery);
    ...
    private static String wrap(String text, int width, String indent) {
        ...
        if (line.length() > 0)
            sb.append(indent).append(line.toString().trim());
        return sb.toString();
    }
    ...
    private static void printHelp() {
        ...
        System.out.println(
                "    " + cyan("bukhari") + " " + yellow("<id>") + "                 " + gray("Get hadith by ID"));
        System.out.println(
                "    " + cyan("bukhari") + " " + yellow("<id>") + " -a" + "              " + gray("Arabic text only"));
        System.out.println(
                "    " + cyan("bukhari") + " " + yellow("<id>") + " -b" + "              " + gray("Arabic + English"));
        System.out.println(
                "    " + cyan("bukhari") + " " + yellow("<id1> <id2> ...") + "         " + gray("Multiple hadiths"));
        System.out.println(
                "    " + cyan("bukhari") + " --search " + yellow("<query>") + "        " + gray("Full-text search"));
        System.out.println(
                "    " + cyan("bukhari") + " --search " + yellow("<query>") + " --all  " + gray("Show all results"));
        System.out.println("    " + cyan("bukhari") + " --chapter " + yellow("<id>") + "        "
                + gray("All hadiths in a chapter"));
